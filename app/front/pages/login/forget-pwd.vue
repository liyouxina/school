<template>
	<view class="box">
		<view class="title mlr-36 size-40 bold">
			{{type==1?'忘记密码':type==2?'重置密码':'设置密码'}}
		</view>
		<view class="mlr-36 size-26 gray-2 mt-10">
			{{type==3?'设置登录密码':'使用手机号验证码找回密码'}}
		</view>
		...
		<view class="mlr-36 mt-10">
			<u-field v-model="pwd" :trim="true" :type="open?'text':'password'" label-width="0" maxlength="16" placeholder="请输入6-16位密码">
				<view slot="icon" class="img-icon">
		...
		<view class="mlr-36 mt-15 size-26 gray-2">
			至于包含数字、字母、符号中的两种元素
		</view>
		<view class="mt-100 plr-36">
			<u-button type="primary" :disabled="!tel || !code || !pwd" @click="submit">{{type==3?'确定':'重置密码'}}</u-button>
		</view>
	</view>
</template>
		...
		methods: {
			...
			submit() {
				if (this.submitbtn == true) {
					...
					this.$http('/addons/ddrive/user/resetpwd', {
						mobile: this.tel,
						captcha: this.code,
